...
<h2><?php echo ((isset($this->_rootref['L_TOPIC_REVIEW'])) ? $this->_rootref['L_TOPIC_REVIEW'] : ((isset($user->lang['TOPIC_REVIEW'])) ? $user->lang['TOPIC_REVIEW'] : '{ TOPIC_REVIEW }')); ?>: <?php echo (isset($this->_rootref['TOPIC_TITLE'])) ? $this->_rootref['TOPIC_TITLE'] : ''; ?></h2>
...
<div id="topicreview">
	...
	<?php $_topic_review_row_count = (isset($this->_tpldata['topic_review_row'])) ? sizeof($this->_tpldata['topic_review_row']) : 0;if ($_topic_review_row_count) {for ($_topic_review_row_i = 0; $_topic_review_row_i < $_topic_review_row_count; ++$_topic_review_row_i){$_topic_review_row_val = &$this->_tpldata['topic_review_row'][$_topic_review_row_i]; ?>
		...
		<div class="post<?php if ($_topic_review_row_val['S_IGNORE_POST']) {  ?> post-ignore<?php } ?>">
			<?php if ($_topic_review_row_val['S_IGNORE_POST']) {  ?><div class="ignore-msg"><?php echo $_topic_review_row_val['L_IGNORE_POST']; ?></div><?php } ?>

		<div class="postbody" id="pr<?php echo $_topic_review_row_val['POST_ID']; ?>">
			<?php if ($_topic_review_row_val['U_MCP_DETAILS'] || ( $_topic_review_row_val['POSTER_QUOTE'] && $_topic_review_row_val['DECODED_MESSAGE'] )) {  ?>

			<ul class="profile-icons linklist actionlist">
				<?php if ($_topic_review_row_val['POSTER_QUOTE'] && $_topic_review_row_val['DECODED_MESSAGE']) {  ?><li class="quote-icon"><a href="#postingbox" onclick="addquote(<?php echo $_topic_review_row_val['POST_ID']; ?>, '<?php echo $_topic_review_row_val['POSTER_QUOTE']; ?>');" title="<?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?> <?php echo $_topic_review_row_val['POST_AUTHOR']; ?>"><?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?> <?php echo $_topic_review_row_val['POST_AUTHOR']; ?></a></li><?php } if ($_topic_review_row_val['U_MCP_DETAILS']) {  ?><li class="quote-icon"><a href="<?php echo $_topic_review_row_val['U_MCP_DETAILS']; ?>"><?php echo ((isset($this->_rootref['L_POST_DETAILS'])) ? $this->_rootref['L_POST_DETAILS'] : ((isset($user->lang['POST_DETAILS'])) ? $user->lang['POST_DETAILS'] : '{ POST_DETAILS }')); ?></a></li><?php } ?>

			</ul>
			<?php } ?>

			<h2><a href="#pr<?php echo $_topic_review_row_val['POST_ID']; ?>"><?php echo $_topic_review_row_val['POST_SUBJECT']; ?></a></h2>
...
<?php if (! $_topic_review_row_val['S_LAST_ROW']) {  ?><hr /><?php } }} ?>
