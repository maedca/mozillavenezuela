<?php if (!defined('IN_PHPBB')) exit; ?><h2><?php echo ((isset($this->_rootref['L_POST_REVIEW'])) ? $this->_rootref['L_POST_REVIEW'] : ((isset($user->lang['POST_REVIEW'])) ? $user->lang['POST_REVIEW'] : '{ POST_REVIEW }')); ?></h2>

<p class="panel"><?php echo ((isset($this->_rootref['L_POST_REVIEW_EXPLAIN'])) ? $this->_rootref['L_POST_REVIEW_EXPLAIN'] : ((isset($user->lang['POST_REVIEW_EXPLAIN'])) ? $user->lang['POST_REVIEW_EXPLAIN'] : '{ POST_REVIEW_EXPLAIN }')); ?></p>
...
<?php $_post_review_row_count = (isset($this->_tpldata['post_review_row'])) ? sizeof($this->_tpldata['post_review_row']) : 0;if ($_post_review_row_count) {for ($_post_review_row_i = 0; $_post_review_row_i < $_post_review_row_count; ++$_post_review_row_i){$_post_review_row_val = &$this->_tpldata['post_review_row'][$_post_review_row_i]; ?>
	...
	<div class="post<?php if ($_post_review_row_val['ONLINE_STATUS']) {  ?> online<?php } if ($_post_review_row_val['S_IGNORE_POST']) {  ?> post-ignore<?php } ?>">
		<?php if ($_post_review_row_val['S_IGNORE_POST']) {  ?><div class="ignore-msg"><?php echo $_post_review_row_val['L_IGNORE_POST']; ?></div><?php } ?>


	<div class="postbody" id="ppr<?php echo $_post_review_row_val['POST_ID']; ?>">
		<h2><a href="#ppr<?php echo $_post_review_row_val['POST_ID']; ?>"><?php echo $_post_review_row_val['POST_SUBJECT']; ?></a></h2>
